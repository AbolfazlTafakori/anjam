...
The installer prints the **administrator username, a generated password and the panel address** once (kept in `/etc/anjam/install-result.env`, root only). Administrators are separate from app users: nobody who signs up in the app can reach the panel. The panel lives at a random path (`/panel-…`) and lets you switch registration between *open / invite only / closed*, create invite codes, make password-reset links, disable/delete users, download a database backup and read the audit log. On the server, `anjam` opens a management menu (`anjam admin reset`, `anjam registration invite`, `anjam invite create`, `anjam backup`, `anjam update`, `anjam log` …). Password-reset e-mails are sent only if `SMTP_URL` is set; otherwise the admin hands out reset links.
