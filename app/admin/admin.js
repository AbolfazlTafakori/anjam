...
(() => {
  ...
  const I18N = {
    fa: {
      ...
      dlAndroidSteps: ['آدرس سرور را در Chrome باز کنید', 'منوی ⋮ → Add to Home screen', 'آیکون «انجام» به صفحه‌ی گوشی اضافه می‌شود؛ آفلاین هم کار می‌کند'], dlWebText: 'همیشه آخرین نسخه؛ نیازی به نصب ندارد.', dlAutoUpdate: 'نسخه‌های ویندوز و لینوکس خودشان به‌روزرسانی را دریافت می‌کنند و در برنامه اعلام می‌کنند.',
    ...
    en: {
      ...
      dlAndroidSteps: ['Open the server address in Chrome', 'Menu ⋮ → Add to Home screen', 'An "Anjam" icon appears on the phone; it works offline too'], dlWebText: 'Always the latest version; nothing to install.', dlAutoUpdate: 'Windows and Linux builds fetch updates themselves and announce them inside the app.',
    ...
    async downloads() {
      ...
      $('#content').innerHTML = `
        ...
        <div class="dl-grid">
          ...
          ${card('android', t('dlAndroid'), `<ol>${t('dlAndroidSteps').map((s) => `<li>${esc(s)}</li>`).join('')}</ol><a class="btn ghost" href="/" target="_blank">${icon('web')}<span>${t('dlOpen')}</span></a>`)}
