...
const { db, q, setting, backupTo } = require('./src/db');
const sec = require('./src/security');
const DATA_DIR = config.dataDir, PUBLIC_URL = config.publicUrl, ADMIN_PATH = config.adminPath;
const scrypt = (pw, salt) => crypto.scryptSync(pw, salt, 64).toString('hex');
const randPass = sec.randomPassword;
...
const regMode = () => setting('registration', config.registrationDefault);
...
switch (`${cmd || ''} ${sub || ''}`.trim()) {
  ...
  case 'users': out(db.prepare('SELECT email, name, disabled, created_at, last_sync_at FROM users ORDER BY created_at').all().map((u) => `${u.email}\t${u.name}\t${u.disabled ? 'DISABLED' : 'active'}\tjoined ${when(u.created_at)}\tsync ${when(u.last_sync_at)}`).join('\n') || '(none)'); break;
  ...
  case 'backup': case `backup ${sub || ''}`.trim(): { const f = sub || path.join(process.env.ANJAM_BACKUP_DIR || DATA_DIR, `anjam-backup-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')}.sqlite`); backupTo(f); out(f); break; }
  case 'stats': { const n = (s) => (db.prepare(s).get() || {}).n || 0; out({ users: n('SELECT COUNT(*) n FROM users'), tasks: n("SELECT COUNT(*) n FROM items WHERE type='task' AND deleted=0"), admins: n("SELECT COUNT(*) n FROM users WHERE role='admin'"), registration: regMode(), db: fs.statSync(path.join(DATA_DIR, 'anjam.sqlite')).size }); break; }
